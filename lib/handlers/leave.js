...
module.exports = function(roomName, fn) {
    ...
    if (roomName) {
        ...
        self.gameroom.pub.publish('gameroom', {rooms: [roomName], channel: 'left', data: {action: 'leave', player: this.login}});
        ...
        this.gameroom.cmd.smembers(socketKey, function(err, rooms) {
            self.gameroom.pub.publish('gameroom', {rooms: rooms, channel: 'left', data: {action: 'leave', player: self.login}});
